      ...
      <el-form :model="operation" :rules="rules" ref="operation" status-icon size="medium" label-width="120px" inline-message>
        <div class="row">
          ...
          <div class="col-lg-6 col-md-6 col-sm-12">
            <el-form-item label="机组成员" prop="flightCrew">
              <el-input v-model="operation.flightCrew" placeholder="" disabled maxlength="200"></el-input>
            </el-form-item>
          </div>
          <div class="col-lg-6 col-md-6 col-sm-12">
        ...
        <el-form-item label="事情经过模板">
          <el-select v-model="select" placeholder="请选择" style="width: 140px;" @change="selectAction">
            <el-option v-for="(item, index) in byData.EVENT_FILGHT_TITLE.dicts" :key="index" :label="item.dictCode" :value="item.dictCode"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 事情简要经过模板
    selectAction () {
      let titles = this.byData.EVENT_FILGHT_TITLE.dicts ? this.byData.EVENT_FILGHT_TITLE.dicts : []
      if (titles.length > 0) {
