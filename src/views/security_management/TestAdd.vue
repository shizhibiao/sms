...
import { conditionGetDepartmentList, getTestProject, getTestConclusion } from '../../services/safety_supervision'
export default {
  components: {
    ...
    saveDataAction () {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid && this.checklistTab.length > 0) {
          // 保存新增事件
          let form = {}
          form.supervisceAuditPlanDO = { ...this.info, details: JSON.parse(JSON.stringify(this.checklistTab)) }
          // console.log(form)
          form.supervisceAuditPlanDO.details.forEach(item => {
          ...
          form.attachementList = this.$refs.adjunct.fileFormList
          // console.log(form)
          axios.post('/api/securitytest/save_atta', (form))
            .then((res) => {
              // console.log(res)
