<template>
 <div class="animated fadeIn">
          ...
          <div class="row">
            ...
            <div class="col-lg-4 col-md-6 col-sm-12">
              <el-form-item label="FC/LEC" title="可能性(F)/严重性(C)、事故发生可能性(F)/人员暴露于危险环境中频繁程度(E)/事故发生可能造成的后果(C)" prop="anonymous">
                <el-radio v-model="operation.anonymous" label='FC' title="可能性(F)/严重性(C)" @click.native.prevent="changeAnonymous('FC')">FC</el-radio>
                <el-radio v-model="operation.anonymous" label='LEC' title="事故发生可能性(F)/人员暴露于危险环境中频繁程度(E)/事故发生可能造成的后果(C)" @click.native.prevent="changeAnonymous('LEC')">LEC</el-radio>
              </el-form-item>
            </div>
    ...
    <b-card no-body>
      <b-card-header>
        根原因
        <el-button size='small' type="success" @click="addRootCause" style="float: right;">增加</el-button>
      </b-card-header>
      <b-card-body>
        <el-table
          size="small"
    ...
    <security-notice-adjunct ref='adjunct' :tableName='tableName' :safetyId='safetyId' v-if="describe == 'compile'" :uploadFile='uploadFile'></security-notice-adjunct>
    <!-- 审核历史 -->
    <b-card no-body v-if="describe === 'handleCompile'">
      <b-card-header>
        审核历史
      ...
      <b-card-body>
        <div style="text-align: center;">
          ...
          <el-button size='small' type="success" @click="sendAction" :disabled="sendActin === false"
            ...
            v-if="describe === 'handleCompile'">发送</el-button>
          <el-button size='small' type="danger" @click="finishFlowAction" v-if="describe === 'handleCompile'">结束流程</el-button>
...
import { getSingleHazardData, removeHazardRootCause, getInvolveCourse, addHazardAtions, getFCLECdata, inquireRootCause, QueryListDataAction, getCompileDataList, compileRiskSource, startHazardStream, disposeHazardAudit, examineAuditHistory } from '../../services/information_report/index.js'

export default {
  name: 'risk_compile_hazard',
  components: {
  ...
  methods: {
    ...
    compileAction () {
      ...
      this.$nextTick(() => {
        compileRiskSource(data).then(result => {
    ...
    // 返回
    gobackAction () {
      let path = this.$route.query.path ? this.$route.query.path : '/risk/hazard-list'
  ...
  created () {
    ...
    // push进来的是哪个
    this.describe = this.$route.query.describe ? this.$route.query.describe : 'compile'
    // 如果是驳回后的新建进来的办理，发送按钮可用
    if (this.describe === 'handleCompile') {
      this.sendActin = true
      this.whetherCount = 0
      // 获取审核历史记录
