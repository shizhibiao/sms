...
export default {
    ...
    methods: {
        querylist() {
            axios.get('/api/incdent/event/listStatisticCurrentPeriod?' + 'startDate=' + this.queryParams.occurDate[0] + '&endDate=' + this.queryParams.occurDate[1] + '&eventGrade=' + this.queryParams.caseLevel)
                .then(res => {
                    ...
                    if (res.data.error === 0) {
                        ...
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                if (!obj[item.caseLevel][item.caseLevel]){
                                    obj[item.caseLevel][item.caseLevel] = item.hj
                                }
                            })
                            let rows = []
                            let num = 0
                            ...
                            let columns = ['caseLevel', ...new Set(caseLevel), '总计']
        ...
        exports () {
            if (this.queryParams.caseLevel === "" || this.queryParams.caseLevel === undefined) {
                this.downLoad(
                'get',
                '/api/incdent/exporce/listStatisticCurrentPeriod?' + 'startDate=' + this.queryParams.occurDate[0] + '&endDate=' + this.queryParams.occurDate[1],
                false,
                '事件等级统计'
                )
            } else {
                this.downLoad(
                'get',
                '/api/incdent/exporce/listStatisticCurrentPeriod?' + 'startDate=' + this.queryParams.occurDate[0] + '&endDate=' + this.queryParams.occurDate[1] + '&eventGrade=' + this.queryParams.caseLevel,
                false,
                '事件等级统计'
                )
            }
