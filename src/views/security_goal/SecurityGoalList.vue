            ...
            <el-form :inline="true" :model="searchForm" class="form-title" ref='searchForm' label-width="80px" >
              <el-row type="flex">
                ...
                <el-col :xs="4" :sm="4" :md="4" :lg="4" :xl="4" align="right" justify="end">
                  ...
                  <el-button type="success" @click="newTarget()" size='small'>新增</el-button>
                </el-col>
...
export default {
  ...
  methods: {
    refresh() {
      ...
      axios.post('/api/target/list',this.form).then(result=>{
        console.log(result)
    ...
    //     this.daterange && this.daterange.length > 1 ? this.daterange[1] : "";
       this.searchForm.deptId = this.deptName && this.deptName.length > 0 ? this.deptName[this.deptName.length - 1] : ''
    },
    //  去重查询数据
    loadData() {
    ...
    deleteRow(index , row){
      let id = row.id
      axios.delete('/api/target/'+id).then(result=>{
